...
func ExtractFancyNumber(fnb FancyNumberBox) int {
	...
	if ok {
		var value, err = strconv.Atoi(fancyNumber.Value())
...
func DescribeFancyNumberBox(fnb FancyNumberBox) string {
	...
	if ok {
		floatNumber, _ := strconv.ParseFloat(fancyNumber.Value(), 64)
		return fmt.Sprintf("This is a fancy box containing the number %.1f", floatNumber)
	}
